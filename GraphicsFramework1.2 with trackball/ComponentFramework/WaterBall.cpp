...
GAME::WaterBall::WaterBall(const Vec3 pos_, const Vec3 orientation_)
{
	pos = pos_;
...
GAME::WaterBall::~WaterBall()
{
	OnDestroy();
}

bool GAME::WaterBall::OnCreate()
{
	shader = new Shader("waterVert.glsl", "waterFrag.glsl", 2, 0, "vVertex", 1, "vNormal");
	textureID[0] = CreateNoise3D();
	return true;
}

void GAME::WaterBall::OnDestroy()
{
	if (shader) delete shader;
}

void GAME::WaterBall::Render() const
{
...
void GAME::WaterBall::Update(const float deltaTime)
{
	// See Entity.h
}

bool GAME::WaterBall::LoadMesh(const char * filename)
{
