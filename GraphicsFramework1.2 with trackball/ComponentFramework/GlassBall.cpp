...
GAME::GlassBall::GlassBall(const Vec3 pos_, const Vec3 orientation_)
{
	pos = pos_;
...
GAME::GlassBall::~GlassBall()
{
	OnDestroy();
}

bool GAME::GlassBall::OnCreate()
{
	shader = new Shader("reflectionVert.glsl", "reflectionFrag.glsl", 2, 0, "vVertex", 1, "vNormal");
	return true;
}

void GAME::GlassBall::OnDestroy()
{
	if (shader) delete shader;
}

void GAME::GlassBall::Render() const
{
...
void GAME::GlassBall::Update(const float deltaTime)
{
	// See Entity.h
}

bool GAME::GlassBall::LoadMesh(const char * filename)
{
